...
timestamp: 2026-07-29T22:24:00Z
---
...
- **Concept Drift (`--drift`)**: Compares the `timestamp` in each concept with the Git last modified timestamp of the associated `resource` path. If the code file is newer than the documentation timestamp, it flags a warning to run `okf-maintain` to sync.
